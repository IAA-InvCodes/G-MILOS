...
__device__ void svdcmp(PRECISION *a, int m, int n, PRECISION w[], PRECISION *v)
{
	...
	for (i=0;i<n;i++) {
	  ...
	  if (i < m) {
		 ...
		 if (scale) {
			 for (k=i;k<m;k++) {
				 PRECISION aux1= a[k*n+i];
				 //a[k*n+i] /= scale;
				 aux1 /= scale;
				 //s += a[k*n+i]*a[k*n+i]; //a[k][i]*a[k][i]
				 s += aux1*aux1; //a[k][i]*a[k][i]
				 a[k*n+i] = aux1;
			 }
			 f=a[i*n+i];
			 g = -SIGN(sqrt(s),f);
...
__device__ void svdcmpf(float *a, int m, int n, float w[], float *v)
{
	...
	for (i=0;i<n;i++) {
	 ...
	 for (k=l;k<n;k++) a[i*n+k] *= scale;
	  ...
	  anorm = MAXF(anorm, fabs(w[i])+fabs(rv1[i]));
	}
	for (i=n-1;i>=0;i--) {
	  if (i < n-1) {
		 if (g) {
...
__device__ float dpythagf(float a, float b)
{
	...
	absa=fabs(a);
	absb=fabs(b);
	if (absa > absb) return absa*sqrt(1.0+ SQRF(absb/absa));
	else return (absb == 0.0 ? 0.0 : absb*sqrt(1.0+ SQRF(absa/absb)));
}
