...
FitsImage *  readFitsSpectroImage (const char * fitsFileSpectra, int forParallel, int nLambdaGrid, int byteOrderGpu){
	fitsfile *fptr;   /* FITS file pointer, defined in fitsio.h */
	FitsImage * image = (FitsImage *) malloc(sizeof(FitsImage));
	...
	if (!fits_open_file(&fptr, fitsFileSpectra, READONLY, &status)){
		...
		if(hdutype==IMAGE_HDU){
			...
			if (!fits_get_img_param(fptr, 4, &bitpix, &naxis, naxes, &status) ){
				...
				for (i = 1; i <= nkeys; i++) {
					char * card = (char *) malloc(FLEN_CARD * sizeof(char));
					char * keyname = (char *) malloc(FLEN_CARD * sizeof(char));
				...
				image->nLambdas=naxes[pos_lambda];
				image->numStokes=naxes[pos_stokes_parameters];
				image->naxes = (long *) calloc(4,sizeof(long));
				...
				numPixelsFitsFile = naxes[pos_row]*naxes[pos_col]*naxes[pos_lambda]*naxes[pos_stokes_parameters];
				// allocate memory to read all pixels in the same array 
				float * imageTemp = (float *) calloc(numPixelsFitsFile, sizeof(float));
				if (!imageTemp)  {
					printf("ERROR ALLOCATION MEMORY FOR TEMP IMAGE");
					return NULL;
          		}
               ...
               return NULL;	
				...
				if(forParallel){
					...
					image->vLambdaImagen = NULL;
					image->pixels = NULL;
					image->spectroImagen = (float *) calloc(image->numPixels*image->nLambdas*image->numStokes, sizeof(float));
					image->spectroImagen2 = (float *) calloc(image->numPixels*image->nLambdas*image->numStokes, sizeof(float));
				}
				else{
					image->pixels = (vpixels *) calloc(image->numPixels, sizeof(vpixels));
					for( i=0;i<image->numPixels;i++){
						image->pixels[i].spectro = (float *) calloc ((image->numStokes*image->nLambdas),sizeof(float));
						image->pixels[i].vLambda = NULL;
						image->pixels[i].nLambda = image->nLambdas;
					}					
					image->vLambdaImagen = NULL;
								...
								for( k=0;k<naxes[1];k++){
									for( h=0;h<naxes[0];h++){
										if(forParallel){
											...
											image->spectroImagen2[ (((i*naxes[2]) + j)*(image->nLambdas*image->numStokes)) + (image->nLambdas * k) + h] = imageTemp [(i*naxes[2]*naxes[1]*naxes[0]) + (j*naxes[1]*naxes[0]) + (k*naxes[0]) + h];  // I =0, Q = 1, U = 2, V = 3
/*											if(byteOrderGpu){
												image->spectroImagen2[ (((i*naxes[2]) + j)*(image->nLambdas*image->numStokes)) + (image->nLambdas * k) + h] = imageTemp [(i*naxes[2]*naxes[1]*naxes[0]) + (j*naxes[1]*naxes[0]) + (k*naxes[0]) + h];  // I =0, Q = 1, U = 2, V = 3
											}
											else
							...
							for( j=0; j<naxes[2];j++){
								for( k=0;k<naxes[1];k++){
									for( h=0;h<naxes[0];h++){
										...
										currentPixel = (currentCol*naxes[pos_row]) + currentRow;
										if(forParallel){
											image->spectroImagen2[(currentPixel *(image->nLambdas*image->numStokes)) + (image->numStokes * currentLambda) + currentStokeParameter] = pixel;
		...
		if(hdutype==IMAGE_HDU){
			...
			if (!fits_get_img_param(fptr, 4, &bitpix, &naxis, naxes, &status) ){
				...
				if(forParallel){
					...
					for( i=0;i<image->numPixels;i++){
						image->pixels[i].spectro = (float *) calloc ((image->numStokes*image->nLambdas),sizeof(float));
						image->pixels[i].vLambda = (PRECISION *) calloc (image->nLambdas, sizeof(PRECISION));
						//image->pixels[i].vLambda = calloc (32, sizeof(PRECISION));
						image->pixels[i].nLambda = image->nLambdas;
					}					
				}
		...
		if(hdutype==IMAGE_HDU){
			...
			if (!fits_get_img_param(fptr, 4, &bitpix, &naxis, naxes, &status) ){
				...
				if(naxis==4){ // image with 4 dimension 
					...
					image->naxes = (long *) calloc(4,sizeof(long));
					image->naxes_original = (long *) calloc(4,sizeof(long));
		...
		if (!fits_get_img_param(fptr, 4, &bitpix, &naxis, naxes, &status) ){
			...
			if(naxis!=1  || naxis!=3){
				if(naxis == 1){ // array of lambads 
					...
					PRECISION  * vAuxLambdas = (PRECISION *) calloc(naxes[0], sizeof(PRECISION));
					long fpixel [1] = {1};
					...
					PRECISION  * vAuxLambdas =(PRECISION *) calloc(numLambdas2Read, sizeof(PRECISION));
...
float * readPerStrayLightFile (const char * perFileStrayLight, int nlambda, PRECISION *  vOffsetsLambda){
	...
	slightPER = (float *) calloc(nlambda*NPARMS,sizeof(float));
	...
	if (!fits_open_file(&fptr, configCrontrolFile->StrayLightFile, READONLY, &status)){
		if (!fits_get_img_param(fptr, 4, &bitpix, &naxis, naxes, &status) ){
			...
			if(naxis==2){
				...
				// READ ALL FILE IN ONLY ONE ARRAY 
				// WE ASSUME THAT DATA COMES IN THE FORMAT ROW x COL x LAMBDA
				slight = (float *) calloc(naxes[0]*naxes[1], sizeof(float));
				long fpixel [2] = {1,1};
				...
				if(*nl_straylight!=naxes[0]){
					float * vStrayLight_aux = (float *) calloc(naxes[0]*naxes[1], sizeof(float));
					for(i=0;i<naxes[0];i++){
						for(j=0;j<naxes[1];j++){
				...
				imageTemp = (float *) calloc(numPixelsFitsFile, sizeof(float));
				if (!imageTemp)  {
					printf("ERROR ALLOCATION MEMORY FOR TEMP IMAGE");
					image= NULL;
					slight = NULL;
				...
				//image->spectroImagen = calloc(image->numPixels*image->nLambdas*image->numStokes, sizeof(float));
				slight = (float *) calloc(image->numPixels*image->nLambdas*image->numStokes, sizeof(float));
		...
		if (!fits_get_img_param(fptr, 2, &bitpix, &naxis, naxes, &status) ){
			...
			if(naxis==2){
				...
				*numCols = naxes[1];
				...
				vMask = (int *) calloc(dimMask, sizeof(int));
				long fpixel [2] = {1,1};
...
int writeFitsImageModels(const char * fitsFile, int numRows, int numCols, Init_Model * vInitModel, float * vChisqrf, int * vNumIterPixel, int addChiqr){
   ...
   if (fits_create_file(&fptr, fitsFile, &status)) /* create new FITS file */
	...
	float * vModel = (float *) calloc(naxes[0] * naxes[1] * naxes[2], sizeof(float));
...
int writeFitsImageModelsSubSet(const char * fitsFile, int numRowsOriginal, int numColsOriginal,  ConfigControl configCrontrolFile, Init_Model * vInitModel, float * vChisqrf, int * vNumIterPixel, int addChiqr){
	...
	float * vModelSub = (float *) calloc(numRowsSub * numColsSub * naxes[2], sizeof(float));
...
int writeFitsImageProfiles(const char * fitsProfileFile, const char * fitsFileOrigin, FitsImage * image){
   ...
   fits_create_file(&outfptr, fitsProfileFile, &status);
	...
	float * outputImage = (float *) calloc(numElemWrite, sizeof(float));
...
int writeFitsImageProfilesSubSet(const char * fitsProfileFile, const char * fitsFileOrigin, FitsImage * image, ConfigControl configCrontrolFile){
   ...
   fits_create_file(&outfptr, fitsProfileFile, &status);
	...
	outputImage = (float *) calloc(numElemWrite, sizeof(float));
...
int writeFitsImageModelsWithArray(char * fitsFile, int numRows, int numCols, PRECISION * eta0, PRECISION * B, PRECISION * vlos, PRECISION * dopp, PRECISION * aa, PRECISION * gm, PRECISION * az, PRECISION * S0, PRECISION * S1, PRECISION * mac, PRECISION * alfa, PRECISION * vChisqrf){
   ...
   if (fits_create_file(&fptr, fitsFile, &status)) /* create new FITS file */
	...
	float * vModel = (float * )malloc(numRows * numCols * NUMBER_PARAM_MODELS* sizeof(float));
