...
# internal flags
NVCCFLAGS   := -O3 -m${TARGET_SIZE} -lineinfo
NVCCFLAGS   += -rdc=true --ftz=true 
#NVCCFLAGS   +=  --default-stream per-thread
#NVCCFLAGS   += --maxrregcount 192
...
LIBRARIES += 
#LIBRARIES += -lcublas -lcufft -lcusolver
#LIBRARIES += -lcublas_device -lcublas -lcudadevrt

################################################################################

LDLIBS= -lm -lcfitsio -lgsl -lgslcblas
SRCDIR= src
OBJ= obj
